...
echo '########################################'
echo 'Setup env variables for Jenkins'
echo '########################################'
echo

source /tmp/env_variables

echo '########################################'
echo 'Wait until Jenkins is available'
echo '########################################'
echo
...
echo '########################################'
echo 'Setup Jenkins Administrator'
echo '########################################'
echo
...
username=$(python3 -c "from urllib.parse import quote; print(quote( input()) )" <<< $SETUP_JENKINS_ADMIN_USERNAME)
new_password=$(python3 -c "from urllib.parse import quote; print(quote( input()) )" <<< $SETUP_JENKINS_ADMIN_PASSWORD )
fullname=$(python3 -c "from urllib.parse import quote; print(quote( input()) )" <<< $SETUP_JENKINS_ADMIN_FULLNAME )
email=$(python3 -c "from urllib.parse import quote; print(quote( input()) )" <<< $SETUP_JENKINS_ADMIN_EMAIL )
...
user=$SETUP_JENKINS_ADMIN_USERNAME
password=$SETUP_JENKINS_ADMIN_PASSWORD
...
url_urlEncoded=$(python3 -c "from urllib.parse import quote; print(quote( input()) )" <<< "$url")
