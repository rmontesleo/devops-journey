# Initial Notes from Tekton



## Resources


### Tekton

- [CLI](https://tekton.dev/docs/cli/)

### Safari
- [Automating Kubernetes with GitOps](https://learning.oreilly.com/course/automating-kubernetes-with/9780138230258/)

### GitHub
- [Tekton CLI](https://github.com/tektoncd/cli)
- [GitHub: Tekton cli releases](https://github.com/tektoncd/cli/releases)


### Docker & Docker Hub
- [DockerHub: Maven](https://hub.docker.com/_/maven)

### Redhat
- [How to use Tekton to set up a CI pipeline with OpenShift Pipelines](https://www.redhat.com/architect/cicd-pipeline-openshift-tekton)

### IBM
- [Introducing Tekton Custom Tasks](https://developer.ibm.com/articles/introducing-tekton-custom-tasks/)


### Solving Issues
- [Task Step: From Command to Script](https://stackoverflow.com/questions/29141436/set-e-and-set-x-in-shell-script)
- [set -e and set -x in shell script](https://stackoverflow.com/questions/29141436/set-e-and-set-x-in-shell-script)